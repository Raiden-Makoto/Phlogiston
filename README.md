...
| **Tier 2** | Independent physics verification: an **ensemble of uMLIPs** (MACE-MP-0, CHGNet, ORB) relaxes each candidate, recomputes an MP-frame `energy_above_hull`, screens phonon dynamical stability, and flags off-distribution candidates via ensemble disagreement | `phlogiston/verify/` (planned; `DESIGN.md`) | GPU (on-box) |
