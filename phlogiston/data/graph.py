...
class CrystalGraph:
    ...
    def __repr__(self) -> str:  # concise
        return (
            f"CrystalGraph(N={self.num_nodes}, E={self.edge_index.shape[1]}, "
...
def structure_to_graph(
    ...
    if not structure.is_ordered:
        raise ValueError(
            "structure_to_graph requires an ordered structure (no partial occupancies)."
        )
    ...
    if covered.size != n:
        ...
        raise ValueError(f"atoms {missing} have no neighbors within {cutoff} A; increase cutoff.")
