...
def _uuu_product(irreps1, irreps2, l_feat: int):
    ...
    tp = o3.TensorProduct(irreps1, irreps2, irreps_out, instr,
...
class SymmetricContraction(nn.Module):
    ...
    def forward(self, A: torch.Tensor) -> torch.Tensor:
        ...
        for i, (tp, out) in enumerate(zip(self.prods, self.to_out)):
...
class Interaction(nn.Module):
    def __init__(self, irreps_in, irreps_sh, irreps_out, l_feat: int = 2,
        ...
        if correlation <= 1:
            ...
            self.msg = SymmetricContraction(self.irreps_A, self.irreps_out,
                                            correlation=correlation, l_feat=l_feat)
